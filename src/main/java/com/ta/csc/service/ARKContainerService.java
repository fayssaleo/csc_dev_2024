...
import static com.ta.csc.intervals.ARKContainerIntervals.*;

@Service
public class ARKContainerService {

    private final ContainerRepository containerRepository;
    public final double hazardousPrice = 23.61;
    public final double transshipmentReeferConnectionPrice = 41;
    public final double transshipmentReeferConnectionPrice_2023 = 45;
    public final double transshipmentReeferConnectionPrice_2024 = 50;
    ...
    public final double emptyContainerPrice = 1.75;

    public ARKContainerService(ContainerRepository containerRepository) {
    ...
    public double calculateReeferConnectionSurcharge(Container container, String month, String year)
        ...
        if (container.isReef() && container.getInvoiceCategory().equals("Transshipment"))
            return numOfDays * gettransshipmentReeferConnectionPrice;
        if (container.isReef() && !container.getInvoiceCategory().equals("Transshipment"))
    ...
    public List<Double> getEmptyContainerPrices(String month, String year) throws ParseException {
        ...
        double price = 0;
        // List<Container>
        // AllARKContainersExpectImport=getAllARKContainers().stream().filter(c->!c.getInvoiceCategory().equals("Import"))
        for (int number : getNumberOfEmptyContainersTeus(month, year, getAllARKContainers())) {
            if (number > 1450) {
                price = DoubleRounder.round((number - 1450) * emptyContainerPrice, 2);
            }
            prices.add(price);
            price = 0;
        }
    ...
    public List<Container> getAllARKContainers() {
        return containerRepository.findByShippingLine("ARK");
    }
