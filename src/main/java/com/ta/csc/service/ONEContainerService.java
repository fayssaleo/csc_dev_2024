...
import static com.ta.csc.intervals.ONEContainerIntervals.*;

@Service
public class ONEContainerService {
    ...
    public ONEContainerService(ContainerRepository containerRepository) {
    ...
    public List<Double> getEmptyContainerPrices(String month, String year) throws ParseException {
        ...
        double price = 0;
        for (int number : getNumberOfEmptyContainersTeus(month, year, getAllONEContainers())) {
            if (number > 500) {
                price = DoubleRounder.round((number - 500) * emptyContainerPrice, 2);
            }
            prices.add(price);
            price = 0;
        }
    ...
    public List<Container> getAllONEContainers() {
        return containerRepository.findByShippingLine("ONE");
    }
    ...
    public List<Interval> getContainerIntervals(Container container) {
        if (container.getFullOrEmpty().equals("Full") && container.getInvoiceCategory().equals("Transshipment"))
            ...
            return (container.getIncDate().getYear() <= 123)
                    ? ((container.getIncDate().getYear() == 122) ? fullIExportIntervals()
                            : (container.getIncDate().getYear() == 123) ? fullIExportIntervals2023()
                                    : fullIExportIntervals2021())
                    : fullIExportIntervals2024();
        return null;
    }
